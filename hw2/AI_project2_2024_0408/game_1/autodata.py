...
class MCTSNode:
    ...
    def simulate_move(self, move):
        ...
        nx, ny = x + dx, y + dy
        while 0 <= nx < 12 and 0 <= ny < 12 and self.mapStat[nx][ny] == 0:
            nx += dx
            ny += dy
...
def explore_region(x, y, mapStat, visited, playerID):
    ...
    if not (0 <= x < 12 and 0 <= y < 12) or visited[x][y] or mapStat[x][y] != playerID:
        return 0
...
def calculate_score(mapStat, playerID):
    visited = [[False] * 12 for _ in range(12)]  # 访问状态矩阵
    total_score = 0
    for x in range(12):
        for y in range(12):
            ...
            for y in range(12):
                if mapStat[x][y] == playerID and sheepStat[x][y] > 1:  # Check cells with player's sheep
                    ...
                    for dx, dy in all_move_dir:
                        ...
                        if 0 <= nx < 12 and 0 <= ny < 12 and mapStat[nx][ny] == 0:
...
def GetStep(mapStat, sheepStat, playerID):
    ...
    mcts(root, playerID, iterations=50)  # 執行100次MCTS迭代
...
def InitPos(mapStat, playerID):
    DOF_array = np.zeros((12, 12))
    DDOF_array = np.zeros((12, 12))
    init_pos = [0,0]
    max_DOF = 0
    max_CDOF = 0
    for i in range(12):
        for j in range(12):
            if mapStat[i][j] == 0:
                ...
                if any(0 <= i + dx <12 and 0 <= j + dy < 12 and mapStat[i + dx][j + dy] == -1 for dx, dy in findBarrier_dir):
                    DOF = 0
                    CDOF = 0
                    for dx, dy in all_move_dir:
                        nx, ny = i + dx, j + dy
                        while 0 <= nx < 12 and 0 <= ny < 12 and mapStat[nx][ny] == 0:
                            DOF += 1
...
# def calculate_DOF(mapStat, playerID):
    L1_DOF_array = np.zeros((12, 12))
    L1_CDOF_array = np.zeros((12, 12))
    distance_array = np.zeros((12, 12))
    enemy_choice = []
    for i in range(12):
        for j in range(12):
            if mapStat[i][j] == 0:
            ...
            if mapStat[i][j] == 0 and L1_DOF_array[i][j] != 0:
                for dx, dy in all_move_dir:
                    ...
                    while 0 <= nx < 12 and 0 <= ny < 12 and mapStat[nx][ny] == 0:
                        ...
                        nx += dx
                        ny += dy
                        # if any(0 <= i + dm <12 and 0 <= j + dn < 12 and mapStat[i + dm][j + dn] == -1 for dm, dn in findBarrier_dir):
...
if __name__ == "__main__":
    process_names = ["MCTS.exe","MCTS_2.exe","MCTS_3.exe","blank.exe"]
    game_N = 100
    score_record = []
    for i in range(game_N):
            ...
            for sub_proc in psutil.process_iter(attrs=['name']):
                    if sub_proc.info['name'] in process_names:
                        try:
                            ...
                            print(f"線程 {sub_proc.info['name']} 已不存在。")
        ...
        mapStat_record = []
        sheepStat_record = []
        laststep_record = MCTSNode(mapStat_record, sheepStat_record, parent=None, move=None)    
        # Game loop
        while True:
            env.update()
            ...
            if env.done:
                ...
                print(f"Game:{i+1}")
                done_mapStat, done_sheepStat = laststep_record.simulate_move(laststep_record.move)
                scores = []
                for i in range(1,5):
                    score = calculate_score(done_mapStat,playerID=i)
                    scores.append(score)
                    print(f"Player1 score:{score}")
                
                indexed_scores = list(enumerate(scores, start=1))
                ...
                rankings = {}
                current_rank = 1  # 当前排名
                last_score = None  # 上一个处理的分数
                skipped_ranks = 0  # 跳过的排名数
                # 生成排名
                for i, (player_id, score) in enumerate(sorted_scores):
                    if score == last_score:
                        ...
                        current_rank += skipped_ranks
                        rankings[player_id] = current_rank
                        skipped_ranks = 1  # 重置跳过的排名数，包括当前这个
                    last_score = score
                # 输出每位玩家的排名，按照玩家ID排序输出
                for player_id in range(1, 5):
                    scores.append(rankings[player_id])
                score_record.append(scores)
                break
            laststep_record.mapStat = env.mapStat
            laststep_record.sheepStat =env.sheepStat
            
            # Get the next move from MCTS
            step = GetStep(env.mapStat, env.sheepStat, env.playerID)
            laststep_record.move = step
    ...
    #save csv
    with open('./data/score_record.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Player 1', 'Player 2', 'Player 3', 'Player 4']) 
        writer.writerows(score_record)

    # 指定颜色，以增加差异
    colors = ['blue', 'green', 'red', 'purple']

    # 游戏场次，X轴
    games = list(range(1, len(score_record) + 1))

    # 指定颜色，以增加差异
    colors = ['blue', 'green', 'red', 'purple']

    # 绘制每个玩家的排名线，使用指定的颜色，反转Y轴以使Rank 1在上方
    scores = np.array(score_record)[:, :4]  # 分数
    rankings = np.array(score_record)[:, 4:]  # 排名
    ...
    games = list(range(1, 5))  # 四个玩家

    # 创建画布和坐标轴 - 平均分数折线图
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(players, average_scores, marker='o', linestyle='-', color='darkblue')
    ax.set_title('Average Scores of Players')
    ax.set_xlabel('Players')
    ax.set_ylabel('Average Score')
    plt.show()
    fig.savefig('./data/Average_Scores_of_Players.png')

    # 创建画布和坐标轴 - 平均排名折线图
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(players, average_rankings, marker='o', linestyle='-', color='darkred')
    ax.set_title('Average Rankings of Players')
    ax.set_xlabel('Players')
    ax.set_ylabel('Average Ranking')
    ax.invert_yaxis()  # 排名越低（数值小）在图中越高
    plt.show()
    fig.savefig('./data/Average_Rankings_of_Players.png')