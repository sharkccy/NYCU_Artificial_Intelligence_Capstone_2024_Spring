    ...
    for x in range(12):
        for y in range(12):
            ...
            if mapStat[x][y] == playerID and not visited[x][y]:
                ...
                net_score += (region_size ** 1.25)
            # 幫助隊友
            if mapStat[x][y] == partner and not visited[x][y]:
                region_size = explore_region(x, y, mapStat, visited, partner)
                net_score += (region_size ** 1.25)
            # 重拳出擊
            # if mapStat[x][y] == playerID and not visited[x][y]:
            #     region_size = explore_region(x, y, mapStat, visited, enemy1)
            #     net_score += (region_size ** 1.25)
            # if mapStat[x][y] == playerID and not visited[x][y]:
            #     region_size = explore_region(x, y, mapStat, visited, enemy2)
            #     net_score += (region_size ** 1.25)
            
    net_score /= 2
    return net_score  # 求和后四舍五入
...
def GetStep(mapStat, sheepStat, playerID):
    ...
    mcts(root, playerID, iterations=500)  # 執行100次MCTS迭代
    '''    
...
def InitPos(mapState):
    InitPos = [0, 0]
    InitPos = calculate_DOF(mapState)
    return InitPos
